...
```{r Figure 2A PCA Plot, warning=FALSE, fig.height=8, fig.width=8, message=FALSE}
...
```{r DE RUVs k1, warning=FALSE, message=FALSE}
...
```{r TopTables DE RUV, warning=FALSE, message=FALSE}
...
```{r Proportion of DEGs, warning=FALSE, message=FALSE}
...
```{r RUVs Overlap of DEGs, warning=FALSE, message=FALSE}
...
The above version was missing 85 shared genes between tx+0 and tx+144 due to shape constraints.

```{r revised fig2c, message=FALSE, warning=FALSE}
venn_prop_RUV_rev <- euler(list(
  "tx+0" = DEG_t0_RUV,
  "tx+24" = DEG_t24_RUV,
  "tx+144" = DEG_t144_RUV
),
...
venn_rev <- plot(venn_prop_RUV_rev,
     fills = list(fill = c("#263CA5", "#5B8FD1", "#89C5E5"), 
                  alpha = 1),
     labels = list(font = 4),
     quantities = list(cex = 0.75),
...
```{r Load in DEGs RUVs, warning=FALSE, message=FALSE}
#Load DEGs Data
...
```{r Scatterplots Timepoint Comparisons, warning=FALSE, message=FALSE}
...
#create a function to prepare scatter data for each pair
...
```{r scatterplots for revisions, include=FALSE, message=FALSE, warning=FALSE}
#in order to ensure that lowly abundant transcripts are not driving this response

# #calculate mean log2CPM per gene from UNFILTERED data
# mean_cpm <- rowMeans(counts_cpm_unfilt) %>%
#   as.data.frame() %>%
#   rownames_to_column("Entrez_ID") %>%
#   setNames(c("Entrez_ID", "mean_log2CPM"))
# 
# #join mean log2CPM to my logFC data
# ma_df <- combined_toptables_dxr_RUV %>%
#   mutate(Entrez_ID = as.character(Entrez_ID)) %>%
#   left_join(mean_cpm %>% mutate(Entrez_ID = as.character(Entrez_ID)),
#             by = "Entrez_ID") %>%
#   drop_na(mean_log2CPM, logFC)
# 
# #efine timepoint colors
# timepoint_colors <- c(
#   "t0"   = "#4167BD",
#   "t24"  = "#5780C5",
#   "t144" = "#72AEE0"
# )
# 
# #make plot per timepoint
# make_cpm_plot <- function(df_sub) {
#   tp <- df_sub$Time[1]
#   
#   ggplot(df_sub, aes(y = mean_log2CPM, x = logFC)) +
...
# }
# 
# #split by timepoint and build plots
# split_tp <- split(ma_df, ma_df$Time)
# cpm_plot_list <- lapply(split_tp, make_cpm_plot)
# 
# # Step 6: Print separately
# ma_plot_list[["t0"]]
# ma_plot_list[["t24"]]
# ma_plot_list[["t144"]]
# 
# # Step 7: Combined versions
# ma_plot_wide <- wrap_plots(ma_plot_list, ncol = length(ma_plot_list))
# ma_plot_tall <- wrap_plots(ma_plot_list, nrow = length(ma_plot_list))
# 
# ma_plot_wide
# ma_plot_tall
```
...
# Figure S6 - Volcano Plots of RUVs DEGs

```{r Volcano Plots RUVs Corrected k1, warning=FALSE, message=FALSE}
