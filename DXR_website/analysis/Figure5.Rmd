...
#read in the ids for the first gene set
...
PDL1pos_HCA2_entrez <- readRDS("data/Fig5/PDL1pos_HCA2_SAGs_entrezid.RDS")
...
cat("PDL1+ HCA2 cells:", length(PDL1pos_HCA2_entrez), "of", length(unique(distefano_s8_ids$Gene_symbol)), "expressed \n")
...
fisher_test_overlap <- function(acute, chronic, degs) {
  ...
  c1 <- sum(chronic %in% degs)
  c2 <- length(chronic) - c1
  a1 <- sum(acute %in% degs)
  a2 <- length(acute) - a1
...
forest_celltype <- ggplot(forest_summary_celltype,
                          aes(y = Gene_Set, x = OR, xmin = CI_low, xmax = CI_high)) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "red") +
  geom_errorbarh(height = 0.2, linewidth = 0.6, color = "black") +
  geom_point(size = 4, color = "black") +
  geom_text(aes(label = Significant), position = position_nudge(y = 0.3), fontface = "bold") +
...
print(plots_list$PDL1pos_HCA2)
print(plots_list$Lung_Epi)
print(plots_list$Fibro)

# save_plot(plots_list$PDL1pos_HCA2,
#           filename = "PropBarplot_Fin_PDL1pos_HCA2_SAGs_AvC_EMP",
#           folder = output_folder
#           )
# 
# save_plot(plots_list$Lung_Epi,
#           filename = "PropBarplot_Fin_LungEpi_SAGs_AvC_EMP",
#           folder = output_folder
#           )
# 
...
```{r sags in motifs proportions sep plots, warning=FALSE, message=FALSE}
#create a function to generate separate plots per cell type for their respective senscence-associated genes - then assess whether they are enriched in Chronic v Acute gene clusters as above in the forest plot

generate_sag_plot <- function(gene_set, set_name) {
  
  #calculate counts within clusters
  count_acute   <- sum(gene_set %in% acute_set)
  count_chronic <- sum(gene_set %in% chronic_set)
  total_captured <- count_acute + count_chronic
  not_in_clusters <- length(gene_set) - total_captured
  
  #perform fisher's exact test
  #background is cluster size vs genes not in cluster
  mat <- matrix(c(count_chronic, length(chronic_set) - count_chronic,
                  count_acute,   length(acute_set) - count_acute), 
                nrow = 2, byrow = TRUE)
  ...
  #form plotting data
  #denominator is all genes found within one motif (not every gene in sets)
  plot_df <- tibble(
    Label = set_name,
    Motif = factor(c("Acute", "Chronic"), levels = c("Chronic", "Acute")),
    Percentage = c((count_acute / total_captured) * 100, 
                   (count_chronic / total_captured) * 100)
  )
  
  #generate plots
  plt <- ggplot(plot_df, 
                aes(x = Label, y = Percentage, fill = Motif)) +
    geom_bar(stat = "identity", color = "black", width = 0.6) +
    geom_text(aes(label = round(Percentage, 1)), 
              position = position_stack(vjust = 0.5), 
              size = 5, fontface = "bold") +
    #add significance stars for Chronic vs Acute enrichment
    annotate("text", x = 1, y = 105, 
             label = sig_label, size = 9, fontface = "bold") +
    scale_fill_manual(values = c("Acute" = "#83C67C", "Chronic" = "#C499C6")) +
    scale_y_continuous(limits = c(0, 115), breaks = seq(0, 100, 20)) +
    labs(
      title = paste("Percentage of SAGs in Chronic vs Acute clusters", set_name),
      subtitle = paste0("Total Set: ", length(gene_set), 
                        " | In Motifs: ", total_captured, 
                        " | Not in Motifs: ", not_in_clusters),
      x = NULL,
      y = "Percent (%)",
      fill = "Cluster"
    ) +
...
#now run my cell type SAG gene sets through to make their respective barplots
#pdl1+ hca2 cells set
PDL1_barplot <- generate_sag_plot(SAG_sets$PDL1pos_HCA2, "PDL1+/nHCA2")
print(PDL1_barplot)

# save_plot(PDL1_barplot,
#           filename = "Barplots_PDL1posHCA2_ChronicvAcute_EMP",
#           folder = output_folder)

epi_barplot <- generate_sag_plot(SAG_sets$Lung_Epi, "Epithelial/nCells")
print(epi_barplot)

# save_plot(epi_barplot,
#           filename = "Barplots_LungEpi_ChronicvAcute_EMP",
#           folder = output_folder)

fibro_barplot <- generate_sag_plot(SAG_sets$Fibro, "Fibroblasts")
print(fibro_barplot)

# save_plot(fibro_barplot,
#           filename = "Barplots_LungFibro_ChronicvAcute_EMP",
#           folder = output_folder)
