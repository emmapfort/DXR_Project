...
# Figure 5D - log2FC Heatmap SAG tx+144 DEGs

```{r Figure 5D, fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
...
sag_distefano <- unique(Lung_Epi_entrez, Fibro_entrez)
#SAG Entrez_IDs
#these are already listed below in the relevant analysis, so no need to print them multiple times
#I already consolidated to a unique set of 2,186 genes across the three gene sets from different cell types used below
entrez_sag <- sag_distefano

#function to extract relevant data
extract_data_sag_RUV <- function(df, name) {
  df %>%
    filter(Entrez_ID %in% entrez_sag) %>%
    mutate(Gene = mapIds(org.Hs.eg.db, 
                         as.character(Entrez_ID),
                         column = "SYMBOL", 
                         keytype = "ENTREZID", 
...
#combine all DEGs and annotate
all_data_sag_RUV <- bind_rows(mapply(extract_data_sag_RUV, deg_list_RUV, 
                                     names(deg_list_RUV), SIMPLIFY = FALSE)) 

#now make a filtering step to only include DEGs at tx+144
sig_genes_t144 <- all_data_sag_RUV %>% 
  dplyr::filter(Condition == "t144", adj.P.Val < 0.05) %>% 
  pull(Gene) %>% 
  unique()

filt_sag_data_t144 <- all_data_sag_RUV %>% 
  dplyr::filter(Gene %in% sig_genes_t144)

#create matrices
logFC_sag_RUV <- acast(filt_sag_data_t144, Gene ~ Condition, 
                          value.var = "logFC")
signif_sag_RUV <- acast(filt_sag_data_t144, Gene ~ Condition, 
                           value.var = "Signif")
...
logFC_mat_sag_RUV <- logFC_sag_RUV[, desired_order]
signif_mat_sag_RUV <- signif_sag_RUV[, desired_order]

#column annotation
timepoint_annot <- colnames(logFC_mat_sag_RUV)

col_annot_sag_RUV <- HeatmapAnnotation(
  Timepoint = timepoint_annot,
  col = list(
    Timepoint = c("t0" = "#005A4C", 
             "t24" = "#328477", 
...
sag_hm_5d <- Heatmap(
  logFC_mat_sag_RUV,
  name = "log2FC",
  top_annotation = col_annot_sag_RUV,
  cluster_columns = FALSE,
  cluster_rows = TRUE,
  show_row_names = TRUE,
  row_names_gp = gpar(fontsize = 7),
  ...
  border = "black",
  layer_fun = function(j, i, x, y, width, height, fill) {
    grid.text(signif_mat_sag_RUV[cbind(i, j)], x, y, gp = gpar(fontsize = 12))
  },
  column_title = "SAG gene response (Epi & Fibro) (Distefano et al.)",
  column_title_gp = gpar(fontsize = 9, fontface = "plain"),
  heatmap_legend_param = list(
          title_gp = gpar(fontsize = 8, fontface = "plain"),
          labels_gp = gpar(fontsize = 7)
