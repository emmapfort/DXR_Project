...
```{r Look at senesence associated genes, message=FALSE, warning=FALSE}
...
```{r degs vs nondegs senescence genes, warning=FALSE, message=FALSE}
#now I will assess the proportion of these senescence-associated genes in my identified DEGs vs nonDEGs

forest_data_sag <- purrr::map_dfr(names(gene_sets), function(gs_name) {
  gs <- unique(na.omit(gene_sets[[gs_name]]))
  
  purrr::map_dfr(names(timepoint_DEGs_ddr), function(tp) {
    degs    <- unique(timepoint_DEGs_ddr[[tp]])
    nondegs <- setdiff(all_genes, degs)
    
    in_deg_yes  <- sum(degs %in% gs)
    in_deg_no   <- length(degs) - in_deg_yes
    in_non_yes  <- sum(nondegs %in% gs)
    in_non_no   <- length(nondegs) - in_non_yes
    
    contingency <- matrix(
      c(in_deg_yes, in_deg_no,
        in_non_yes, in_non_no),
      nrow = 2, byrow = TRUE
    )
    
    fisher_res <- fisher.test(contingency)
    
    cat(sprintf("[%s | %s]  OR=%.3f  p=%.5f  DEGs: %d/%d  nonDEGs: %d/%d\n",
                gs_name, tp,
                unname(fisher_res$estimate), fisher_res$p.value,
                in_deg_yes, length(degs),
                in_non_yes, length(nondegs)))
    ...
    tibble(
      ...
      Prop_DEG    = sprintf("%d/%d", in_deg_yes, length(degs)),
      Prop_nonDEG = sprintf("%d/%d", in_non_yes, length(nondegs))
    )
  })
})

#factor ordering
forest_data_sag <- forest_data_sag %>%
  mutate(
    GeneSet   = factor(GeneSet,   levels = names(SAG_sets)),
    Timepoint = factor(Timepoint, levels = c("t0", "t24", "t144")),
    Sig_Label = ifelse(Significant, "Significant", "Not Significant")
...
#create the plot
plot_forest_SAGset <- ggplot(forest_data_sag,
                              aes(y = Timepoint, x = Odds_Ratio,
                                  color = GeneSet, shape = Sig_Label)) +
  geom_point(size = 4) +
  geom_errorbarh(aes(xmin = CI_low, xmax = CI_high), height = 0.25) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "gray50") +
  geom_text(aes(x = CI_high, label = paste0("DEG: ", Prop_DEG)),
  ...
  scale_x_log10() +
  scale_y_discrete(limits = rev(c("t0", "t24", "t144"))) +
  facet_wrap(~ GeneSet, ncol = 1) +
  labs(
    title  = "Senescence Associated Genes in DEGs vs nonDEGs",
    y      = "Timepoint",
    x      = "Odds Ratio (95% confidence interval)",
    color  = "Gene Set",
    shape  = "Significance"
