...
#3. perform a Fisher's exact test for enrichment in a given gene set
#we are testing whether DAPs are enriched in Acute vs Chronic sets
...
#4. create a summary table for plotting barplots
plot_summary_genes <- map_dfr(names(comparison_genes), function(gene_set) {
  genes   <- comparison_genes[[gene_set]]
  fisher_res <- fisher_test_overlap(acute_set, chronic_set, genes)

  acute_overlap <- sum(acute_set %in% genes)
  acute_nonoverlap <- length(acute_set) - acute_overlap
  chronic_overlap <- sum(chronic_set %in% genes)
  chronic_nonoverlap <- length(chronic_set) - chronic_overlap

  tibble(
    GeneSet  = gene_set,
    Motif       = rep(c("Acute", "Chronic"), each = 2),
    Category    = rep(c("DAP", "nonDAP"), 2),
    Count       = c(acute_overlap,
                    acute_nonoverlap,
                    chronic_overlap,
                    chronic_nonoverlap),
    Fraction    = c(acute_overlap,
                    acute_nonoverlap,
                    chronic_overlap,
                    chronic_nonoverlap) /
                  c(length(acute_set),
                    ...
                    length(chronic_set),
                    length(chronic_set)),
    Significant = rep(c(fisher_res$significant, ""), 2)
  )
})

plot_summary_genes$Category <- factor(plot_summary_genes$Category, 
                                      levels = c("DAP", "nonDAP"))

label_positions_genes <- plot_summary_genes %>%
  filter(Category == "DAP") %>%
  mutate(y_pos = 1)

#5. create barplots with the results of the fisher's exact test
#these look at the proportion of DAPs and nonDAPs in Acute and Chronic sets
dap_barplot_motifs_genes <- ggplot(plot_summary_genes, aes(x = GeneSet, 
                                                           y = Fraction, 
                                                           fill = Category)) +
  geom_bar(stat = "identity", position = "stack") +
  geom_text(data = label_positions_genes %>% 
              filter(Significant != ""),
            aes(x = GeneSet, 
                y = y_pos, 
                label = Significant),
  ...
  facet_wrap(~Motif) +
  labs(
    title = "Percentage of DAPs vs nonDAPs mapping to Acute and Chronic gene clusters",
    x     = "Protein Set",
    y     = "Percentage of Cluster Genes (%)",
    fill  = ""
  ) +
  theme_custom()

print(dap_barplot_motifs_genes)

save_plot(dap_barplot_motifs_genes, 
          filename = "DAPvnonDAP_AcutevChronic_Barplots_EMP",
          folder = output_folder)

#print the summary table for statistics
for (gene_set in names(comparison_genes)) {
  genes           <- comparison_genes[[gene_set]]
  acute_overlap   <- sum(acute_set %in% genes)
  chronic_overlap <- sum(chronic_set %in% genes)
  prop_ac         <- acute_overlap   / length(acute_set)
  ...
  cat(sprintf("%s - Acute: %.2f%% (%d/%d), Chronic: %.2f%% (%d/%d),
              Fisher p = %.5f\n",
              gene_set,
              100 * prop_ac, acute_overlap, length(acute_set),
              100 * prop_ch, chronic_overlap, length(chronic_set),
              fisher_res$p_value))
}
