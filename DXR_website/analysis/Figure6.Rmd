...
#read in SAG sets
SAG_sets <- list(
  PDL1pos_HCA2 = readRDS("data/Fig5/PDL1pos_HCA2_SAGs_entrezid.RDS"),
  Lung_Epi     = readRDS("data/Fig5/Lung_epi_SAGs_entrezid.RDS"),
  Fibro        = readRDS("data/Fig5/Lung_fibro_SAGs_entrezid.RDS")
)

SAG_sets$Combined_SAG <- unique(unlist(SAG_sets))
...
plot_order <- c("Combined_SAG", "PDL1pos_HCA2", "Lung_Epi", "Fibro")
...
make_forest_plot_clean <- function(df) {
  ggplot(df, aes(y = Category, x = OR_plot, xmin = CI_low_plot, xmax = CI_high_plot)) +
    geom_point(size = 4) +
...
pairwise_overlaps <- data.frame(
  Comparison = c("PDL1pos vs Lung_Epi", "PDL1pos vs Fibro", "Lung_Epi vs Fibro"),
  Shared_Genes = c(
    length(intersect(SAG_sets$PDL1pos_HCA2, SAG_sets$Lung_Epi)),
    length(intersect(SAG_sets$PDL1pos_HCA2, SAG_sets$Fibro)),
    length(intersect(SAG_sets$Lung_Epi, SAG_sets$Fibro))
  )
)
