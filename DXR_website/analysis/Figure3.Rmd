...
#       panel.grid.minor = element_line(color = "grey80", linewidth = 0.5)   #grey minor grid for align in illus
#     )
# }

# saveRDS(plot_theme_custom, "data/plot_theme_custom_update.RDS")
...
# Figure S7 - AIC BIC Number of Clusters
...
# Figure S9 - mean CPM of each transcript over time

```{r Revised FigS9 version to show meanCPM, message=FALSE, warning=FALSE}

counts_cpm_unfilt <- readRDS("data/QC/counts_cpm_unfilt.RDS")

mean_cpm <- rowMeans(counts_cpm_unfilt) %>%
  as.data.frame() %>%
  rownames_to_column("Entrez_ID") %>%
  setNames(c("Entrez_ID", "mean_log2CPM"))

cpm_df <- combined_toptables_dxr_RUV %>% 
  ...
  left_join(mean_cpm %>% mutate(Entrez_ID = as.character(Entrez_ID)),
            by = "Entrez_ID") %>% 
  drop_na(mean_log2CPM, logFC) %>% 
  mutate(cluster = case_when(
    Entrez_ID %in% as.character(final_genes_1_RUV) ~ "Acute",
    Entrez_ID %in% as.character(final_genes_2_RUV) ~ "Chronic",
    TRUE ~ "Other"
  ),
  cluster = factor(cluster, levels = c("Other", "Acute", "Chronic"))) %>% 
  arrange(cluster)

#ensure that I have the correct amount of genes for each cluster
cpm_df %>% 
  dplyr::distinct(Entrez_ID, cluster) %>% 
  dplyr::count(cluster)
#7,602 Acute genes
#501 Chronic genes

cpm_df_clusters <- cpm_df %>% 
  dplyr::filter(cluster != "Other") %>%
  mutate(cluster = factor(cluster, levels = c("Acute", "Chronic")))


cluster_means <- cpm_df_clusters %>%
  dplyr::group_by(cluster, Timepoint) %>%
  dplyr::summarise(mean_logFC = mean(logFC, na.rm = TRUE),
                   mean_log2CPM = mean(mean_log2CPM, na.rm = TRUE),
                   .groups = "drop")

mean_line_cols <- c(
  "Acute"   = "#2E8B57",  #darker green
  "Chronic" = "#7B3F9E"   #darker purple
)


make_cpm_plot <- function(df_sub) {
  tp <- df_sub$Timepoint[1]
  means_sub <- cluster_means %>% dplyr::filter(Timepoint == tp)
  
  ggplot(df_sub, aes(x = mean_log2CPM, y = logFC, color = cluster)) +
    geom_point_rast(alpha = 0.7, size = 2, raster.dpi = 300) +
    #mean logFC lines per cluster as horizontal lines at mean CPM position
    geom_hline(data = means_sub %>% dplyr::filter(cluster == "Acute"),
               aes(yintercept = mean_logFC),
               color = mean_line_cols["Acute"], linewidth = 1) +
    geom_hline(data = means_sub %>% dplyr::filter(cluster == "Chronic"),
               aes(yintercept = mean_logFC),
               color = mean_line_cols["Chronic"], linewidth = 1) +
    scale_color_manual(values = motif_cols) +
    labs(
      x = "mean log2CPM",
      y = paste0("log2FC ", tp, " (DOX vs. VEH)"),
      color = "Cluster",
      title = tp
    ) +
    xlim(0, 15) +
...
timepoint_order <- c("tx+0", "tx+24", "tx+144")

cpm_df_clusters <- cpm_df_clusters %>%
  mutate(Timepoint = factor(Timepoint, levels = timepoint_order))

split_cpm <- split(cpm_df_clusters, cpm_df_clusters$Timepoint)
cpm_plot_list <- lapply(split_cpm, make_cpm_plot)

cpm_plot_list[["tx+0"]]
cpm_plot_list[["tx+24"]]
cpm_plot_list[["tx+144"]]

cpm_plot_wide <- wrap_plots(cpm_plot_list, ncol = length(cpm_plot_list),
                           guides = "collect")
cpm_plot_tall <- wrap_plots(cpm_plot_list, nrow = length(cpm_plot_list),
                           guides = "collect")

cpm_plot_wide
cpm_plot_tall

# save_plot(cpm_plot_wide,
#           filename = "meanCPM_logFC_Scatterplot_EMP_260316",
#           folder = output_folder,
#           height = 12,
#           width = 20)

# save_plot(cpm_plot_tall,
#           filename = "meanCPM_logFC_tall_Scatterplot_EMP_260316",
#           folder = output_folder,
#           height = 20,
#           width = 12)

# save_plot(cpm_plot_list$`tx+0`,
#           filename = "meanCPM_Supp9_t0_EMP",
#           folder = output_folder,
#           height = 4,
#           width = 5.5)
# 
# save_plot(cpm_plot_list$`tx+24`,
#           filename = "meanCPM_Supp9_t24_EMP",
#           folder = output_folder,
#           height = 4,
#           width = 5.5)
# 
# save_plot(cpm_plot_list$`tx+144`,
#           filename = "meanCPM_Supp9_t144_EMP",
#           folder = output_folder,
#           height = 4,
#           width = 5.5)
...
# Figure S8 - Chronic (n=501) Line Plot
...
motif1_mat_GOKEGG_RUV <- gost(query = acute_genes_matrix,
                          ...
                          significant = TRUE,
                          custom_bg = all_genes,
                          correction_method = c("fdr"), #in this case p_value = adj.p
...
table_motif1_GOKEGG_genes_RUV <- motif1_mat_GOKEGG_RUV$result %>% 
  dplyr::select(c(source, term_id, term_name, 
...
table_motif2_GOKEGG_genes_RUV <- motif2_mat_GOKEGG_RUV$result %>%
  dplyr::select(c(source, term_id, term_name, intersection_size, 
                  term_size, p_value, intersection))
...
table_motif2_genes_GOBP_RUV %>% ggplot(., aes(x = log_val, y = reorder(term_name, p_value), col= p_value)) +
  geom_point(aes(size = intersection_size)) +
  ggtitle("M2 Enriched GO:BP Terms RUVs")+
  xlab(expression("-log"[10]~"p-value"))+
  guides(size= guide_legend(title = "# of intersected \n terms"),
         col = guide_legend(title = "adj. p-value"))+
  ylab("GO:BP term")+
  scale_y_discrete(labels = scales::label_wrap(30))+
  theme_bw()+
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5),
...
table_motif2_genes_KEGG_RUV <- table_motif2_GOKEGG_genes_RUV %>% 
  dplyr::filter(source == "KEGG") %>% 
  dplyr::select(source, term_id, term_name, intersection_size, 
                  term_size, intersection, p_value) %>% 
  dplyr::slice_min(., n = 10, order_by = p_value) %>% 
  mutate(log_val = -log10(p_value))

table_motif2_genes_KEGG_RUV %>% ggplot(., aes(x = log_val, y = reorder(term_name, p_value), col= p_value)) +
  geom_point(aes(size = intersection_size)) +
...
We have revised this figure to include the adj. p value (color) and the number of genes in each term (size). 

```{r Fig 3E 3F Revisions, message=FALSE, warning=FALSE}

#plot the revised versions with adj. p as color and intersection_size as size of points
kegg_m1_plot_rev <- table_motif1_genes_KEGG_RUV %>%
...
kegg_m2_plot_rev <- table_motif2_genes_KEGG_RUV %>%
  ggplot(aes(x = log_val, y = reorder(term_name, p_value), col = p_value)) +
  geom_point(aes(size = intersection_size)) +
  ggtitle("Enriched Pathways: Chronic") +
  xlab(expression("-log"[10]~"adj. p-value")) +
...
print(kegg_m1_plot_rev)
print(kegg_m2_plot_rev)

#now save these plots for figure generation

# save_plot(kegg_m1_plot_rev,
#           filename = "Fig3E_Acute_KEGG_Revised_EMP",
#           folder = output_folder,
#           height = 5,
#           width = 5.25)

# save_plot(kegg_m2_plot_rev,
#           filename = "Fig3F_Chronic_KEGG_Revised_EMP",
#           folder = output_folder,
#           height = 5,
#           width = 5.25)
...
#acute cluster (motif1)
# cairo_pdf(
#   file.path(output_folder, "KEGG_Acute_Revised_Fig3E_EMP_260310.pdf"),
#   width = 8,
#   height = 8
# )
# print(kegg_m1_plot_rev)
...
#chronic cluster (motif2)
# cairo_pdf(
#   file.path(output_folder, "KEGG_Chronic_Revised_Fig3F_EMP_260310.pdf"),
#   width = 8,
#   height = 8
# )
# print(kegg_m2_plot_rev)
...
# Figure 3E + 3F - Acute & Chronic Gene Tables

```{r Fig 3E 3F Gene tables function, warning=FALSE, message=FALSE}
#create a function to pull out intersection data and make gene tables
...
make_KEGG_gene_table <- function(data, n_terms = 10, output_file = NULL) {
  ...
  #step 5 - reorder for clarity based on adj. p values
...
# Acute_m1_KEGG_gene_table <- make_KEGG_gene_table(
#   table_motif1_GOKEGG_genes_RUV,
#   output_file = "Fig3E_Acute_gene_table_EMP_260310.csv")

# saveRDS(Acute_m1_KEGG_gene_table, "data/Fig3/Fig3E_Acute_KEGG_gene_list.RDS")

#read in the final version for Fig 6B
Acute_m1_KEGG_gene_table <- readRDS("data/Fig3/Fig3E_Acute_KEGG_gene_list.RDS")

# Chronic_m2_KEGG_gene_table <- make_KEGG_gene_table(
#   table_motif2_GOKEGG_genes_RUV,
#   output_file = "Fig3F_Chronic_gene_table_EMP_260310.csv")

# saveRDS(Chronic_m2_KEGG_gene_table, "data/Fig3/Fig3F_Chronic_KEGG_gene_list.RDS")

Chronic_m2_KEGG_gene_table <- readRDS("data/Fig3/Fig3F_Chronic_KEGG_gene_list.RDS")

#print the gene tables
print(Acute_m1_KEGG_gene_table)
print(Chronic_m2_KEGG_gene_table)
